...
function isOpenRouterReady(): boolean {
...
async function generateSimulatedChatWithOpenRouter(query: string, latestScreenBase64?: string): Promise<string> {
  try {
...
async function generateChatWithGroq(messageText: string, history: any[]): Promise<{ userTranscript: string; aiResponse: string }> {
  try {
    const messages = [
      {
        role: "system",
...
app.get("/api/config", (req, res) => {
  ...
  let orMask = "";
  if (orKey) {
    orMask = orKey.length > 8 
      ? `${orKey.slice(0, 8)}...${orKey.slice(-4)}` 
      : "***";
  }
...
// YouTube support was removed as requested.
...
app.post("/api/chat-voice", async (req, res) => {
  ...
  // PRIMARY CHOICE: If OpenRouter API is ready, handle with highest preference!
  if (isOpenRouterReady()) {
    try {
      ...
      return res.json({
        ...
        audioBase64: "", // Clients do speech synthesis locally using Web Speech Synthesis API
        ttsFallback: true,
  ...
  if (useFallback) {
    if (isOpenRouterReady()) {
      try {
        ...
        let aiResponse = orResult.aiResponse;
  ...
  try {
    ...
    // TTS generation in chat has been disabled as requested
...
// 2. Direct TTS Endpoint (disabled as requested)
...
wss.on("connection", async (clientWs: WebSocket, request: any) => {
              ...
              onclose: () => {
                ...
                if (!dataReceived) {
                  ...
                  clientWs.send(JSON.stringify({
                    type: "status",
                    data: "Jonli muloqot ulanishida uzilish bo'ldi. Simulyator muloqot rejimi muvaffaqiyatli faollashtirildi. Jasurbek AI savollaringizga ovozli va matnli javob qaytarishga tayyor! 🎤"
                  }));
                } else {
  ...
  if (isSimulated) {
    ...
    clientWs.send(JSON.stringify({ 
      type: "status", 
      data: `Jonli muloqot ulanishida xatolik yuz berdi${errorDetail}. Simulyator rejimi faollashtirildi. Jasurbek AI savollaringizga ovozli va matnli javob qaytarishga tayyor! 🎤`
    }));
  }
    ...
    try {
      ...
      if (isSimulated) {
        if (parsed.type === "text" && parsed.data) {
          ...
          // User transcription for visualization
          clientWs.send(JSON.stringify({ type: "user-transcription", text: query, data: query }));
          
          // Generate conversational response through general REST API (prioritizing OpenRouter)
          let aiText = "";
